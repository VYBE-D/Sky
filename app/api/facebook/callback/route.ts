...
export async function GET(request: NextRequest) {
  ...
  const { error } = await supabase.from('facebook_connections').upsert({ workspace_id: workspace.id, provider: 'meta', account_name: me.name, status: 'connected', access_token_encrypted: token.access_token, token_expires_at: token.expires_in ? new Date(Date.now() + token.expires_in * 1000).toISOString() : null, permissions: pages.data?.map((p: any) => p.tasks ?? []) ?? [], last_sync: new Date().toISOString() }, { onConflict: 'workspace_id' })
  if (error) return NextResponse.redirect(new URL('/settings/integrations?facebook=save_error', request.url))
  const { data: connection } = await supabase.from('facebook_connections').select('id').eq('workspace_id', workspace.id).single()
  for (const page of pages.data ?? []) await supabase.from('facebook_pages').upsert({ workspace_id: workspace.id, connection_id: connection?.id, page_id: page.id, name: page.name, category: page.category }, { onConflict: 'page_id' })
